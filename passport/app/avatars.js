/* Avatar builder: layered animal avatars with per-animal ANCHORS so
   ...
   API on window.STI: avatarParts, avatarSrc(cfgOrSeed), avatarFor(handle), randomAvatar(seed) */
(function () {
  // [bg, head, dark]
  const COLORS = [
  ...
  const ANIMALS = [
    { name: "Cat", anchors: { eyes: [[25.5, 34], [38.5, 34]], hatY: 18, ear: [16.5, 42], nose: [32, 40], cheek: [44, 43] }, draw: (h, d) => `
    ...
    { name: "Bear", anchors: { eyes: [[25.5, 34], [38.5, 34]], hatY: 19, ear: [14.5, 43], nose: [34.8, 44.6], cheek: [44.5, 43.5] }, draw: (h, d) => `
    ...
    { name: "Fox", anchors: { eyes: [[25, 33], [39, 33]], hatY: 18, ear: [15.5, 41], nose: [34.6, 47.2], cheek: [45, 39] }, draw: (h, d) => `
    ...
    { name: "Frog", anchors: { eyes: [[22, 22], [42, 22]], hatY: 8, ear: [14.5, 45], nose: [34.4, 36.5], cheek: [44, 47] }, draw: (h, d) => `
    ...
    { name: "Bunny", anchors: { eyes: [[26, 35], [38, 35]], hatY: 13, ear: [17.5, 43], nose: [34.2, 39.5], cheek: [43.5, 44] }, draw: (h, d) => `
    ...
    { name: "Owl", anchors: { eyes: [[25, 35], [39, 35]], hatY: 18, ear: [15.5, 43], nose: [34.6, 44], cheek: [45, 43] }, draw: (h, d) => `
  ...
  const GLASSES = [
    ...
    { name: "Shades", draw: (h, d, a) => {
      const [[x1, y1], [x2, y2]] = a.eyes;
      const lens = (x, y) => `<path d="M${x - 7} ${y - 3.4} h14 v2.4 q0 6-7 6 q-7 0-7-6 Z" fill="${d}" opacity="0.92"/>`;
      const by = (y1 + y2) / 2 - 3.4;
      return lens(x1, y1) + lens(x2, y2) +
  ...
  const EXTRAS = [{ name: "None", draw: () => "" }];

  const PARTS = { animals: ANIMALS, colors: COLORS, hats: HATS, glasses: GLASSES, extras: EXTRAS };
  ...
  function avatarSvg(cfgIn) {
    const cfg = normalize(cfgIn);
    const [bg, head, dark] = COLORS[cfg.color];
    const animal = ANIMALS[cfg.animal];
    const a = animal.anchors;
    return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64">` +
      `<rect width="64" height="64" fill="${bg}"/>` +
      animal.draw(head, dark) +
      GLASSES[cfg.glasses].draw(head, dark, a) +
      HATS[cfg.hat].draw(head, dark, a) +
      EXTRAS[cfg.extra].draw(head, dark, a) +
      `</svg>`;
  }

  function avatarSrc(cfg) {
    return "data:image/svg+xml," + encodeURIComponent(avatarSvg(cfg));
  }

  function randomAvatar(seed) {
    let s = (seed >>> 0) || 1;
    const next = (n) => { s = (s * 1103515245 + 12345) & 0x7fffffff; return s % n; };
    return {
  ...
  window.STI.avatarParts = PARTS;
  window.STI.avatarSrc = avatarSrc;
  window.STI.avatarFor = avatarFor;
  window.STI.randomAvatar = randomAvatar;
})();
