...
**6. Data minimization.** The best protection against losing data is not having it. No account to *view* a shared status. Per-context aliases, unlinkable across apps. **No real names — display identity is a handle/alias + avatar, and no name field exists anywhere in the system** (a name is a collection surface the product has no use for). No central identity index (amended by [Reach and Access](16-reach-and-access.md): the one allowed index is an opt-in, status-free `name -> aliasId` vanity directory for findable sharing). No analytics/ad SDKs near health surfaces. Consent at creation, working delete/revoke, minimal retention.
