...
`/push/register`, `/healthz`, `/`) or the catch-all `other` (into which every route outside that
template set collapses: the inbox, `/u`, `/recovery`, `/g`, `/republish`, `/vapid`, `/feedback`,
`/admin/*`, and anything else), **never** the concrete path, so an id can never become a label value. That single discipline is the difference between safe and catastrophic, so it is called out
again in the audit.
...
| `feedback_received_total` | counter | none | "Something wrong?" reports filed ([doc 35](35-something-wrong-reports.md)). A rise between scrapes is what makes the box email the operator a bare "N new report(s)" nudge; the count is all that leaves, never a category or note. |
