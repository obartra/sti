...
Status: BUILT (non-extractable `RootKey`, `rootKeyStore`, resumable session, and the keep-signed-in toggle all shipped).

## Why

Reloading the page logs the owner out. The unlocked root lives only in React
...
root as a **non-extractable WebCrypto `CryptoKey`** in IndexedDB.
...
- Non-extractable: the key can be USED on the device (it derives the account id,
  ...
  script injected into the page cannot copy the root out to reuse elsewhere.
  That closes the worst case (root theft) without pretending an in-page
...
## The root key lifecycle

Today `root` is raw `Bytes` threaded through the account/session layers. It
becomes a non-extractable `CryptoKey` (`RootKey`), used directly by HKDF. The
...
Raw root key bytes still exist transiently, only at the moments a secret enters:
account create (phrase -> PBKDF2), recover (phrase), and passkey unlock (PRF ->
unwrap). At each, the bytes are imported once into the non-extractable `RootKey`
and dropped. After that, no layer holds raw bytes.

The one consequence: the passkey enroll used to wrap `session.root` (raw bytes).
...
1. `crypto/keys.ts`: `RootKey = CryptoKey`; `importRootKey(bytes)` (HKDF,
   extractable `false`); `deriveAccountId/Key/WriteToken` take a `RootKey`.
   `deriveRootKey(phrase)` still returns the transient bytes.
2. `root: Bytes -> RootKey` through `account.ts`, `accountSync.ts`,
   `session.ts`, `OwnerSession`.
3. `enrollPasskey` re-derives bytes from the recovery phrase rather than reading
   `session.root`.
4. New `rootKeyStore` (IndexedDB): `save(RootKey)`, `load()`, `clear()`.
...
- An in-page (XSS) compromise can still act as the owner while the page is open;
  it just cannot exfiltrate the root. Non-extractable bounds the blast radius,
...
- Derivation compatibility: id/blob-key/write-token from a `RootKey` equal the
  values the old byte path produced (so existing accounts unlock).
- `rootKeyStore` round-trips a non-extractable key and `clear()` removes it;
