...
| Recovery envelopes | `{opaque_account_id, factor} → ciphertext` | One per opt-in unlock factor (passkey, optional password): the account's root key wrapped under that factor's own secret (doc 32). The server holds only the wrapped ciphertext, never a secret, and the account id derives from the root key, never from a password. |
