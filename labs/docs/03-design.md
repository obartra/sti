...
- Within any alias there are only **two observable states**, never a leaky third, because each alias is one mode. (The "gray-with-labels vs gray-without-labels" third state only appears if you run both intents on one alias — don't; mint separate aliases.)
...
- **Public profiles can't be existence-hidden and are scrapeable over time** (a scheduled scraper reconstructs cadence and gray episodes). Mitigations, not a full fix: opt-in, default-off, time-boxed; login-gated even for "public" so resolution is rate-limitable; coarse temporal precision ("as of [month]"); transition hysteresis (blue→gray flips on a fuzzy/batched schedule, decorrelated from the health event); per-id/per-viewer rate-limiting; and an honest in-app warning that a public profile is watchable.

### Knock — stranger access to a private profile, without going public or leaking existence

A private alias can be **advertised** (its link put on a dating profile / bio) while keeping the *status* gated. Someone who holds the link but isn't authorized can **knock** — request access — and the owner decides. This is NOT a third sharing mode: it's just what *private* does when a viewer holds the link but not the key. There is one private mode.
...
- Sentence (~6th–8th grade, active voice, no jargon): e.g. *"[handle] says they've tested
