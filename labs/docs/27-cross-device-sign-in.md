...
section C. Pairs with [Data & storage](09-data-and-storage.md) (the account master is the account),
...
own phone. One constraint shapes the build: the live master is now a non-extractable key that cannot be
...
1. **Move the master, not the passkey.** The account is the account master
   ([keyVault.ts](../../passport/src/auth/keyVault.ts); account id, blob key, and write tokens are all
   HKDF'd from it). A passkey is only a local credential whose PRF output wraps a copy of the master
   ([passkey.ts](../../passport/src/auth/passkey.ts)). So pairing never tries to copy a passkey across
   devices (it cannot cross platforms anyway). It moves the master once, and each phone then mints its
...
- **The old phone (the authorizer).** It holds the account, but the live master is a non-extractable
  key it cannot read out ([stay signed in](24-stay-signed-in.md)). So to show the code it does a fresh
  unlock (its passkey, a Face ID prompt, or the recovery phrase), which is the one moment raw master
...
1. **Old phone:** from the profile, tap **"Add a device."** Because the live master cannot be exported,
   it first asks for a fresh unlock (passkey/Face ID, or the recovery phrase); that unlock briefly
   re-mints the raw master. It then shows a short warning and a QR carrying it (a versioned payload, for
   example `stcd1:` plus the base64url master), and drops the bytes again. Copy (voice to finalize):
...
3. **New phone finishes:** it derives the account id, opens the account blob from `GET /acct/{id}` to
   ...
   half-works), then enrolls its **own** local passkey over the master and lands signed in.
...
The QR carries the account master itself, so a single scan is enough: no key exchange, no return
...
- **Native local transports (same ecosystem, optional polish):** on native, the QR can be replaced by a
  ...
  Connections on Android, or an NFC tap (doc 26 section E). The payload is the same **sealed** master,
...
- **A stolen, already-unlocked old phone** cannot just reveal the code: the live master is
...
- **Two-scan ephemeral exchange.** The new phone shows an ephemeral public key, the old phone seals the
  master to it and shows it back, so neither QR is a secret. It removes the bearer-secret risk entirely
  but needs **two scans** ("turn the phone around"), which is the friction we are explicitly avoiding.
- **One-scan relay.** The new phone shows an ephemeral public key, the old phone seals the master and
